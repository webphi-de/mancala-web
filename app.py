...
def mache_zug_api(mulden_index):
    ...
    steine_in_mulde = brett.mulden[mulden_index]
    zug_ergebnis = brett.mache_zug(mulden_index)
    log_eintrag = f"Mensch: Mulde {mulden_index + 1} ({steine_in_mulde}) -> landet in {zug_ergebnis['letzter_index']}"
    ...
    while aktueller_spieler == ki.spieler_nummer and not brett.pruefe_spielende():
        ...
        steine_in_mulde_ki = brett.mulden[ki_zug_index]
        zug_ergebnis_ki = brett.mache_zug(ki_zug_index)
        log_eintrag = f"KI: Mulde {mulden_index - 6} ({steine_in_mulde_ki}) -> landet in {zug_ergebnis['letzter_index']}"
        spiel_verlauf.append(log_eintrag)
